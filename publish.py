...
def zip_file(workspace, eve_app):
    pomFilePath = '%s/%s/code/pom.xml' % (
        workspace, eve_app)
    if os.path.isfile(pomFilePath):
        os.rename(pomFilePath, pomFilePath.replace(".xml", ""))
        cachedRepoPath = '%s/.m2' % (workspace)
        targetRepoPath = '%s/%s/code/.m2' % (workspace, eve_app)
        shutil.copytree(cachedRepoPath, targetRepoPath)
        javaFilePath = '%s/%s/code/src/main/java/com/aliyun/openservices/Demo.java' % (
            workspace, eve_app)
        os.rename(javaFilePath, javaFilePath.replace(".java", ""))
